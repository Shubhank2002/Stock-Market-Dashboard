...
const Sidebar = () => {
    ...
    const GET_Company_List=async()=>{
        try {
            const res=await axios.get('https://stock-market-dashboard-backend-zmxm.onrender.com/stock')
            if(res.data){
    ...
    const handleClick=async(item)=>{
        if(selected===item.Symbol){
            ...
            try {
                const newstockdetails=await axios.get(`https://stock-market-dashboard-backend-zmxm.onrender.com/stock/${item.Symbol}`)
